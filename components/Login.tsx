...
/** Rasmiy Tergov Departamenti logosi (qalqon, qilich, O'zbekiston gerbi). */
...
const LOCKOUT_MESSAGE = "Kirish vaqtincha cheklangan. 10 daqiqadan keyin qayta urinib ko'ring yoki tizim administratoriga murojaat qiling.";
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const simulateLoginProcess = () => {
    const steps = [
        "Server bilan himoyalangan aloqa o'rnatilmoqda...",
        "Foydalanuvchi ma'lumotlari shifrlanmoqda...",
        "Biometrik identifikatsiya ma'lumotlari tekshirilmoqda...",
    ...
    setTimeout(() => {
      if (username === validUsername && password === validPassword) {
        ...
        if (next >= MAX_FAILED_ATTEMPTS) {
          ...
          setError(`Login yoki parol noto'g'ri. Qolgan urinish: ${MAX_FAILED_ATTEMPTS - next}.`);
        }
      }
    }, 800);
  };
    ...
    <div className="w-screen h-screen flex items-center justify-center bg-[#F8FAFC] font-sans overflow-hidden relative selection:bg-uzblue/10 selection:text-uzblue">
      ...
      <div className="relative z-10 w-full max-w-[1100px] h-[80vh] bg-white border border-slate-200 rounded-[2.5rem] shadow-2xl flex overflow-hidden ring-4 ring-slate-50/50">
          ...
          <div className="hidden lg:flex w-1/2 flex-col justify-between p-12 relative overflow-hidden bg-slate-50/50">
              ...
              <div className="space-y-8 relative z-10">
                  ...
                  <div className="flex items-center gap-5">
                      <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-md flex items-center justify-center">
                          <img src={LOGO_SRC} alt="O'zbekiston Respublikasi IIV Tergov Departamenti" className="w-24 h-28 object-contain drop-shadow-sm" />
                      </div>
                      <div className="py-2">
                          <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tight leading-none mb-2">Tergov <br/><span className="text-uzblue">Departamenti</span></h1>
                          <div className="flex items-center gap-3">
                             <div className="h-0.5 w-6 bg-uzblue rounded-full"></div>
                             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Farg'ona viloyati</p>
                          </div>
                      </div>
                  </div>
                  ...
                  <div className="space-y-4 pt-6">
                      <h2 className="text-3xl font-black text-slate-800 leading-tight">
                          <span className="text-uzblue">Ziyrak AI</span> <br/>
                          Tahlil Tizimi
                      </h2>
                      <p className="text-slate-500 text-sm leading-relaxed max-w-sm font-medium">
                          Jinoyatlarni ochishda sun'iy intellekt imkoniyatlaridan foydalanish, 
                          raqamli dalillarni tahlil qilish va tergov jarayonini avtomatlashtirish 
                          uchun yagona milliy platforma.
                      </p>
                  </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 relative z-10">
                  {[
                      { icon: BrainCircuit, label: "AI Tahlil", color: "text-uzblue" },
                      { icon: Fingerprint, label: "Kriminalistika", color: "text-emerald-600" },
                      { icon: Shield, label: "Kiberhimoya", color: "text-amber-500" },
                      { icon: Globe, label: "Global Qidiruv", color: "text-purple-600" }
                  ].map((item, i) => (
          ...
          <div className="w-full lg:w-1/2 bg-white flex flex-col justify-center px-12 lg:px-20 relative">
              ...
              <div className="max-w-sm w-full mx-auto space-y-8 relative z-10">
                  ...
                  <div className="text-center lg:text-left">
                       ...
                       <div className="lg:hidden mb-8 flex flex-col items-center">
                          <div className="p-4 bg-white rounded-3xl shadow-lg border border-slate-100 mb-4 flex items-center justify-center">
                              <img src={LOGO_SRC} alt="O'zbekiston Respublikasi IIV Tergov Departamenti" className="w-24 h-28 object-contain" />
                          </div>
                          <h2 className="text-lg font-black text-slate-900 uppercase leading-tight mb-1">Tergov Departamenti</h2>
                       </div>

                      <h2 className="text-2xl font-black text-slate-800 mb-2">Tizimga Kirish</h2>
                      <p className="text-slate-500 text-xs font-bold uppercase tracking-wide">Shaxsiy identifikatsiya ma'lumotlaringiz</p>
                  </div>
                  ...
                  <form onSubmit={handleLogin} className="space-y-5">
                      ...
                      <div className="space-y-1.5">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
                              ...
                              {capsLock && <span className="text-amber-500 flex items-center gap-1"><AlertCircle size={10}/> CAPS LOCK ON</span>}
                          </label>
                          <div className="relative group">
                              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      ...
                      <div className="flex items-center justify-between pt-2">
                          <label className="flex items-center gap-2 cursor-pointer group select-none">
                              ...
                              <span className="text-xs font-bold text-slate-500 group-hover:text-slate-700">Eslab qolish</span>
                          </label>
                          <a href="#" className="text-xs font-bold text-uzblue hover:text-blue-700 transition-colors">Yordam kerakmi?</a>
                      </div>
                          ...
                          {loading && (
                              <div className="absolute inset-0 bg-blue-700 flex flex-col items-center justify-center z-10 transition-all">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <span className="text-[10px] text-white animate-pulse">{loadingStep || "Tizimga ulanish..."}</span>
                                  </div>
                  ...
                  <div className="pt-6 border-t border-slate-100 flex items-center justify-center gap-2">
                       ...
                       <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">SSL 256-bit Shifrlangan Aloqa</span>
                  </div>
              </div>
          </div>
      </div>
      ...
      <div className="absolute bottom-6 text-center w-full z-10">
           <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] hover:text-slate-600 transition-colors cursor-default">
               &copy; 2026 O'zbekiston Respublikasi Ichki Ishlar Vazirligi
           </p>
      </div>
    </div>
  );
