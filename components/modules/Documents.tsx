...
const Documents: React.FC<DocumentsProps> = ({ onBack, initialSearch = "", onClearInitialSearch }) => {
  ...
  const confirmDelete = useCallback(() => {
    ...
    try {
      ...
      toast(err instanceof Error ? err.message : "O'chirishda xatolik", "error");
    } finally {
      setDeleteTargetId(null);
    }
  }, [deleteTargetId, selectedDoc?.id, refreshDocs, toast]);
    ...
    <div className="w-full h-full flex flex-col bg-[#F8FAFC] overflow-hidden text-slate-900">
        <div className="h-20 border-b border-slate-200 bg-white flex items-center justify-between px-6 shrink-0 z-20 shadow-sm">
            <div className="flex items-center gap-4">
                <button type="button" onClick={onBack} className="p-3 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-500 transition-all border border-slate-200" aria-label="Orqaga"><ArrowLeft size={22}/></button>
                <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3 tracking-tight"><Archive className="text-amber-500" size={28}/> ARXIV</h2>
            </div>
            <div className="bg-slate-100 p-2 rounded-xl flex items-center border border-slate-200">
                ...
                <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Qidiruv..." className="bg-transparent outline-none text-sm text-slate-800 w-48 min-w-0" aria-label="Hujjatlar bo'ylab qidiruv"/>
            </div>
        </div>
        ...
        <div className="flex flex-1 min-h-0 p-6 gap-6">
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                {filteredDocs.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 text-slate-500">
                        ...
                        <p className="text-sm font-bold">{docs.length === 0 ? "Hozircha hujjatlar yo'q" : "Qidiruv bo'yicha natija topilmadi"}</p>
                        <p className="text-xs mt-1">{docs.length === 0 ? "Stenogramma, protokol yoki boshqa modullardan saqlang." : "Boshqa so'z bilan qidiring."}</p>
                    </div>
                ) : (
                    ...
                    {filteredDocs.map((doc) => (
                        <div key={doc.id} onClick={() => setSelectedDoc(doc)} role="button" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && setSelectedDoc(doc)} className={`p-5 rounded-xl border cursor-pointer hover:shadow-lg transition-all ${selectedDoc?.id === doc.id ? "border-uzblue bg-blue-50" : "border-slate-200 bg-white hover:border-slate-300"}`}>
                            ...
                            <div className="flex justify-between mt-4 items-center">
                                ...
                                <button type="button" onClick={(e) => deleteDoc(doc.id, e)} className="p-1.5 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-50" aria-label="O'chirish"><Trash2 size={16}/></button>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <ConfirmModal
              ...
              message="Ushbu hujjat arxivdan butunlay o'chiriladi. Qaytarib bo'lmaydi. Davom etasizmi?"
            ...
            {selectedDoc && (
                <div className="w-[450px] bg-white border border-slate-200 rounded-2xl shadow-xl flex flex-col overflow-hidden animate-slide-in-right shrink-0">
                    <div className="p-6 border-b border-slate-200 flex justify-between items-center">
                        ...
                        <button type="button" onClick={() => setSelectedDoc(null)} className="p-2 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-100" aria-label="Yopish"><X size={20}/></button>
                    </div>
